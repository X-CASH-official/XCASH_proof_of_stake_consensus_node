...
int reset_variables_allocated_on_the_heap_test()
{
  ...
  if (settings2 == 1)
  {
    ...
    for (count = 0; count <= 1000; count++)
    {
      fprintf(stderr,"Current progress for get_previous_block_template: %zu / 1000",count);
      fprintf(stderr,"\r");
      memset(data_test,0,strnlen(data_test,BUFFER_SIZE));
      get_previous_block_template(data_test,0);
      if (count == 0)
      {    
        current_memory_usage = get_program_memory_usage(process_id_file) - previous_system_memory_usage;
      }
      if (count == 10)
      {
        ...
        if ((current_system_memory_usage - previous_system_memory_usage) > current_memory_usage * 9 && current_memory_usage > 0)
        {
          color_print("FAILED! get_previous_block_template has not reset all variables allocated on the heap","red");
          settings2 = 0;
          break;
        }  
      } 
      ...
      if (count == 1000)
      {
        ...
        {
          color_print("PASSED! get_previous_block_template has reset all variables allocated on the heap","green");
          count_test++;
        }      
      } 
    }
  ...
  if (settings2 == 1)
  {
    ...
    for (count = 0; count <= 1000; count++)
    {
      fprintf(stderr,"Current progress for get_delegate_online_status: %zu / 1000",count);
      fprintf(stderr,"\r");
