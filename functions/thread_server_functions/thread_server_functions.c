  ...
  for (;;)
  {
    ...
    while (update_block_verifiers_timer_settings.settings == 1)
    {
      ...
      {  
        ...
        // enable all of the votes in the database
        // set the collection
        ...
        while (mongoc_cursor_next(document_settings, &current_document))
        {
          ...
          // create the message
          memcpy(settings,"{\"public_address_created_reserve_proof\":\"",44);
          memcpy(settings+44,data3,XCASH_WALLET_LENGTH);
          memcpy(settings+142,"\"}",2);
              
          // update the database
          if (update_document_from_collection(DATABASE_NAME,"votes_list",settings,"{\"vote_status\":\"enabled\"}",0) == 0)
          {
            color_print("Could not update a delgates vote status in the database\nFunction: update_block_verifiers_timer","red");
          }    

          // load all of the valid reserve proofs into the reserve_proofs_list struct
          // get the public_address_created_reserve_proof
          memset(data2,0,strnlen(data2,BUFFER_SIZE));
          memset(data3,0,strnlen(data3,BUFFER_SIZE));
          memcpy(data2,", \"",3);
          ...
          // get the total
          memcpy(data2,", \"",3);
          memcpy(data2+3,"total",13);
          memcpy(data2+16,"\" : \"",5);

          message_copy1 = strstr(data,data2) + strnlen(data2,BUFFER_SIZE);
          message_copy2 = strstr(message_copy1,"\"");
          memcpy(data3,message_copy1,message_copy2 - message_copy1);
          sscanf(data3, "%lld", &reserve_proofs_list.amount[reserve_proofs_list.count]);
